"""Treinamento do modelo de finalizacoes (XGBoost).

Mesma estrutura do modelo de escanteios, mas preve total de finalizacoes.
"""
import json
import warnings
...
class ShotsTrainer:
    """Treinador do modelo de finalizacoes com XGBoost."""
    ...
    def train(self, test_games: int = 380, retrain_full: bool = True) -> dict:
        logger.info(f"[Shots] Gerando dataset para {self.league or 'todas ligas'}...")
        ...
        if X.empty:
            logger.error("[Shots] Dataset vazio")
            return {}

        self.feature_names = list(X.columns)
        logger.info(f"[Shots] Dataset: {len(X)} amostras, {len(self.feature_names)} features")
        ...
        logger.info(f"[Shots] Treino: {len(X_train)} | Teste: {len(X_test)}")
        ...
        logger.info(f"[Shots] MAE: {self.metrics['mae_test']:.2f} | "
        ...
        feat_imp = sorted(zip(self.feature_names, importance),
                          key=lambda x: -x[1])
        logger.info("[Shots] Top 5 features:")
        ...
        if retrain_full and len(X_test) > 0:
            logger.info("[Shots] Retreinando com todos os dados...")
            self.model = xgb.XGBRegressor(**{k: v for k, v in params.items()
                                              if k != "early_stopping_rounds"})
    ...
    def load(self) -> bool:
        if not self.paths["model"].exists():
            logger.warning(f"[Shots] Modelo nao encontrado em {self.paths['model']}")
            return False
        ...
        logger.info(f"[Shots] Modelo carregado ({self.metrics.get('mae_test', '?')} MAE)")
        return True
