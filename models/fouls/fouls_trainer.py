"""Treinamento do modelo de faltas (Fouls) com XGBoost.

Treina um XGBRegressor para prever total de faltas por partida,
...
class FoulsTrainer:
    """Treinador do modelo de faltas com XGBoost."""
    ...
    def train(self, test_games: int = 380,
        ...
        logger.info(f"[Fouls] Gerando dataset para {self.league or 'todas ligas'}...")
        ...
        if X.empty:
            logger.error("[Fouls] Dataset vazio")
            return {}

        self.feature_names = list(X.columns)
        logger.info(f"[Fouls] Dataset: {len(X)} amostras, {len(self.feature_names)} features")
        ...
        if len(X_train) < 100:
            logger.warning("[Fouls] Poucos dados de treino, usando params mais simples")
            params["max_depth"] = 3
            params["n_estimators"] = 100

        logger.info(f"[Fouls] Treino: {len(X_train)} | Teste: {len(X_test)}")
        ...
        logger.info(f"[Fouls] MAE: {self.metrics['mae_test']:.2f} | "
        ...
        logger.info("[Fouls] Top 5 features:")
        ...
        if retrain_full and len(X_test) > 0:
            logger.info("[Fouls] Retreinando com todos os dados...")
            self.model = xgb.XGBRegressor(**{k: v for k, v in params.items()
                                              if k != "early_stopping_rounds"})
    ...
    def _save(self):
        ...
        if self.model:
            ...
            logger.info(f"[Fouls] Modelo salvo em {self.paths['model']}")
    ...
    def load(self) -> bool:
        ...
        if not self.paths["model"].exists():
            logger.warning(f"[Fouls] Modelo nao encontrado em {self.paths['model']}")
            return False
        ...
        logger.info(f"[Fouls] Modelo carregado ({self.metrics.get('mae_test', '?')} MAE)")
        return True
