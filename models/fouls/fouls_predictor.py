"""Preditor de faltas (Fouls) usando modelo XGBoost treinado.

Gera previsoes de total de faltas e probabilidades
...
class FoulsPredictor:
    """Preditor de faltas para partidas."""

    def __init__(self, league: str | None = None):
        self.league = league
        self.trainer = FoulsTrainer(league=league)
        self._loaded = False
    ...
    def predict(self, home_team: str, away_team: str,
        ...
        """Preve total de faltas para uma partida."""
        self._ensure_loaded()
        if not self._loaded or self.trainer.model is None:
            logger.error("[Fouls] Modelo nao carregado")
            return None
        ...
        if df.empty:
            logger.warning(f"[Fouls] Sem dados historicos para {league}")
            return None
        ...
        if feats["home_total_games"] < 2 or feats["away_total_games"] < 2:
            logger.warning(f"[Fouls] Times com pouco historico: "
        ...
        if nan_cols:
            logger.debug(f"[Fouls] NaN features: {nan_cols}")
            X_df = X_df.fillna(X_df.mean())

        pred = float(self.trainer.model.predict(X_df.values)[0])

        calib = self.trainer.calib
        residual_std = calib.get("residual_std", 2.0)

        lines = [20.5, 22.5, 24.5, 26.5, 28.5]
        probs = {}
        for line in lines:
    ...
    def predict_many(self, fixtures: list[dict]) -> list[dict]:
        ...
        for fx in fixtures:
            try:
                ...
                logger.warning(f"[Fouls] Erro prevendo {fx.get('home_team')}x{fx.get('away_team')}: {e}")
        return results
