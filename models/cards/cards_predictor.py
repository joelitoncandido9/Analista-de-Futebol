"""Preditor de cartoes amarelos usando modelo XGBoost treinado.

Gera previsoes de total de cartoes amarelos e probabilidades
...
class CardsPredictor:
    """Preditor de cartoes amarelos para partidas."""

    def __init__(self, league: str | None = None):
        self.league = league
        self.trainer = CardsTrainer(league=league)
        self._loaded = False
    ...
    def predict(self, home_team: str, away_team: str,
        ...
        """Preve total de cartoes amarelos para uma partida."""
        self._ensure_loaded()
        if not self._loaded or self.trainer.model is None:
            logger.error("[Cards] Modelo nao carregado")
            return None
        ...
        if df.empty:
            logger.warning(f"[Cards] Sem dados historicos para {league}")
            return None
        ...
        if feats["home_total_games"] < 2 or feats["away_total_games"] < 2:
            logger.warning(f"[Cards] Times com pouco historico: "
        ...
        if nan_cols:
            logger.debug(f"[Cards] NaN features: {nan_cols}")
        ...
        return {
            ...
            "predicted_total_yellow": round(pred, 2),
            "probabilities": probs,
            "calib_std": round(residual_std, 2),
        }

    def predict_many(self, fixtures: list[dict]) -> list[dict]:
        """Preve cartoes amarelos para multiplas partidas."""
        results = []
        for fx in fixtures:
            try:
                pred = self.predict(
                ...
                logger.warning(f"[Cards] Erro prevendo {fx.get('home_team')}x{fx.get('away_team')}: {e}")
        return results
