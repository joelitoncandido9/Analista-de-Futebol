"""Treinamento do modelo de cartoes amarelos com XGBoost.

Treina um XGBRegressor para prever total de cartoes amarelos por partida,
...
class CardsTrainer:
    """Treinador do modelo de cartoes amarelos com XGBoost."""
    ...
    def train(self, test_games: int = 380,
        ...
        logger.info(f"[Cards] Gerando dataset para {self.league or 'todas ligas'}...")

        X, y = build_dataset(
            league=self.league,
            target_col="total_yellow",
            min_data_games=5,
        )

        if X.empty:
            logger.error("[Cards] Dataset vazio")
            return {}

        self.feature_names = list(X.columns)
        logger.info(f"[Cards] Dataset: {len(X)} amostras, {len(self.feature_names)} features")
        ...
        if len(X_train) < 100:
            logger.warning("[Cards] Poucos dados de treino, usando params mais simples")
            params["max_depth"] = 3
            params["n_estimators"] = 100

        logger.info(f"[Cards] Treino: {len(X_train)} | Teste: {len(X_test)}")
        ...
        logger.info(f"[Cards] MAE: {self.metrics['mae_test']:.2f} | "
        ...
        logger.info("[Cards] Top 5 features:")
        ...
        if retrain_full and len(X_test) > 0:
            logger.info("[Cards] Retreinando com todos os dados...")
            self.model = xgb.XGBRegressor(**{k: v for k, v in params.items()
                                              if k != "early_stopping_rounds"})
    ...
    def _save(self):
        ...
        if self.model:
            ...
            logger.info(f"[Cards] Modelo salvo em {self.paths['model']}")
    ...
    def load(self) -> bool:
        ...
        if not self.paths["model"].exists():
            logger.warning(f"[Cards] Modelo nao encontrado em {self.paths['model']}")
            return False
        ...
        logger.info(f"[Cards] Modelo carregado ({self.metrics.get('mae_test', '?')} MAE)")
        return True
